...
void rot_x(data_t *data) {
	...
	while (y < data->height) {
		x = 0;
		while (x < data->width) {
			data->map[y][x].xp = data->map[y][x].xp * zoom + data->pos.x;
			data->map[y][x].yp = (data->map[y][x].yp * cos(0.123599) + data->map[y][x].z * sin(0.123599)) * zoom + data->pos.y;
			data->map[y][x].z = (-data->map[y][x].yp * sin(0.123599) + data->map[y][x].z * cos(0.123599)) * zoom + data->pos.x;
			x++;
		}
		y++;
	}
...
void		apply_height(data_t *e, int c)
{
	...
	while (y < e->height)
	{
		while (x < e->width)
		{
			...
			{
				if (c == 1)
					e->map[y][x].z = e->map[y][x].z0 + 1; //* e->alt);
				else
					e->map[y][x].z = e->map[y][x].z0 -1;// * e->alt);
				e->map[y][x].xp = ((cos(0.523599) * (x - y)) * zoom) + e->pos.x ;
				e->map[y][x].yp = ((-e->map[y][x].z + ((x + y) * sin(1.723599))) * zoom) + e->pos.y;
			}
			x++;
		}
		x = 0;
